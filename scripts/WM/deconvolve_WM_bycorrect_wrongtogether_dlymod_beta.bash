...
prefix=WM_correct_load_wrongtogether_dlymod_betaseries
...
3dDeconvolve \
    ...
    -num_stimts  7 \
        -stim_times_IM 1 $oneddir/cue_ld1_Correct.1D "$dmodel" -stim_label 1 cue_ld1 \
        -stim_times_IM 2 $oneddir/cue_ld3_Correct.1D "$dmodel" -stim_label 2 cue_ld3 \
        -stim_times_IM 3 $oneddir/delay_ld1_Correct.1D "$dmodel" -stim_label 3 delay_ld1 \
        -stim_times_IM 4 $oneddir/delay_ld3_Correct.1D "$dmodel" -stim_label 4 delay_ld3 \
        -stim_times_IM 5 $oneddir/probe_ld1_Correct.1D "$pmodel" -stim_label 5 probe_ld1 \
        -stim_times_IM 6 $oneddir/probe_ld3_Correct.1D "$pmodel" -stim_label 6 probe_ld3 \
	-stim_times_IM 7 $oneddir/Wrong_sort.1D "$pmodel" -stim_label 7 wrong \
        -overwrite \
        -x1D Xmat_${prefix}.x1D -xjpeg dm_$prefix.png \
	-tout -fitts fitts_${prefix} -errts errts_${prefix} -bucket stats_${prefix}

#then we need to extract out the betas for each period
#this needs edited
