...
dmodel="SPMG3"#"dmBLOCK(1)"    # cue is a variable length block covering cue, ISI & mem; variable length, but fixed amplitude
#ldmodel#="BLOCK(3,1)"    # long  delay is 3s
#sdmodel=#"BLOCK(1,1)"    # short delay is 1s
pmodel="SPMG3" #"BLOCK(1,1)"     # average response time is 1s


#need to sort the ones that were incorrect
timing_tool.py -timing $oneddir/Wrong.1D -sort -write_timing $oneddir/Wrong_sort.1D

prefix=WM_correct_load_wrongtogether_dlymod_SPMG3
...
3dDeconvolve \
    ...
    -num_stimts  7 \
        -stim_times_AM1 1 $oneddir/cue_ld1_incCatch_Correct.1D "$dmodel" -stim_label 1 cue_ld1 \
        -stim_times_AM1 2 $oneddir/cue_ld3_incCatch_Correct.1D "$dmodel" -stim_label 2 cue_ld3 \
        -stim_times_AM1 3 $oneddir/delay_ld1_incCatch_Correct.1D "$dmodel" -stim_label 3 delay_ld1 \
        -stim_times_AM1 4 $oneddir/delay_ld3_incCatch_Correct.1D "$dmodel" -stim_label 4 delay_ld3 \
	...
	-stim_times 7 $oneddir/Wrong.1D "$pmodel" -stim_label 7 wrong \
        -overwrite \
