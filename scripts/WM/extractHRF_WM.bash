...
subj1dDir=/Volumes/Phillips/P5/group_analyses/WM/subj1d/extractHRF_allsepdly
...
for roi in $rois; do
   for condition in $conditions; do
	   ...
	   output=${subj1dDir}/${d}_${condition}_${roi}.1D 
	   #echo ${d}
	   #echo ${output}
	   3dROIstats -mask $roidir/${roi}_10mm+tlrc -quiet ${subjdir}/${d}/contrasts/WM/${condition}+tlrc |cat -n | sed "s/^/$d $condition $roi/" | tee ${output}
	   #3dROIstats -quiet -mask $currmask $subbrick_l | cat -n | sed "s/^/$id $bart long $i /"
	  
       done
       cat ${subj1dDir}/*_${condition}_${roi}.1D > ${subj1dDir}/${condition}_${roi}_group.1D
   done
