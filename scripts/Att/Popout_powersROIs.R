...
source("..txt/WM/graphing_functions.R")
...
Popout<-read.delim(file="../txt/Popout_powersROIs_mean.txt",header=T)
...
labels<-read.delim(file="../txt/Att/power_roi_labels.txt",header=F)
colnames(Popout.4)<-labels$V1

#subj<-read.delim(file="/Volumes/Phillips/P5/scripts/SubjInfoGoogleSheet_wdx.txt")
subj<-read.delim(file="../txt/SubjInfoGoogleSheet_wdx.txt")
