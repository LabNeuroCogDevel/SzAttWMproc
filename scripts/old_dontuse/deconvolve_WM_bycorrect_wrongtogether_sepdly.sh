...
dmodel="dmBLOCK"    # cue is a variable length block covering cue, ISI & mem; variable length, but fixed amplitude
...
3dDeconvolve \
    ...
    -num_stimts  9 \
        ...
        -stim_times 3 $oneddir/delay_ld1_dly1_Correct.1D "$sdmodel" -stim_label 3 delay1_ld1 \
        -stim_times 4 $oneddir/delay_ld1_dly3_Correct.1D "$ldmodel" -stim_label 4 delay3_ld1 \
	 -stim_times 5 $oneddir/delay_ld3_dly1_Correct.1D "$sdmodel" -stim_label 5 delay1_ld3 \
        -stim_times 6 $oneddir/delay_ld3_dly3_Correct.1D "$ldmodel" -stim_label 6 delay3_ld3 \
