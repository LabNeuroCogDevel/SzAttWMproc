...
table_correct<-function(filename){
  ...
  avg_RT_ld3<-mean(a$RT[ is.finite(a$RT) & a$ld==3 & a$Crt==1])  
  ...
  avg_RT_ld3_delay1<-mean(a$RT[ is.finite(a$RT) & a$ld==3 & a$islongdelay==0 & a$Crt==1])  
  avg_RT_ld3_delay3<-mean(a$RT[ is.finite(a$RT) & a$ld==3 & a$islongdelay==1 & a$Crt==1])  
  ...
  #ld 3, delay1
  ld3_delay1_cor<-sum(a$Crt == 1 & a$ld  == 3 &a$islongdelay==0, na.rm=TRUE)
  ld3_delay1_inc<-sum(a$Crt == 0 & a$ld  == 3 &a$islongdelay==0, na.rm=TRUE)
  ld3_delay1_nors<-sum(a$Crt == -1 & a$ld  == 3 &a$islongdelay==0, na.rm=TRUE)
  ...
  #ld 3, delay3
  ld3_delay3_cor<-sum(a$Crt == 1 & a$ld  == 3 &a$islongdelay==1, na.rm=TRUE)
  ld3_delay3_inc<-sum(a$Crt == 0 & a$ld  == 3 &a$islongdelay==1, na.rm=TRUE)
  ld3_delay3_nors<-sum(a$Crt == -1 & a$ld  == 3 &a$islongdelay==1, na.rm=TRUE)
...
data_table<-subset(data_table, !(ld1_per_cor<50 | ld3_per_cor <50)) #exclude individuals that have <50% correct on one of the loads
#data_table<-data_table[c(1:2,5:19,21:27,29:32,34:37),]



#read in google doc
subj<-read.delim(file="//10.145.64.109/Phillips/P5/scripts/SubjInfoGoogleSheet.txt",header=T)
...
data_table2<-data_table2[,c(1:28,36)]
  ...
  
{
  ...
  model_beh<-glm(data_table2[[i]]~as.factor(subj2$Cohort),data=data_table2)
  print((summary(model_beh)$coefficients[2,4]))
}


colnames(data_table2)<-c("subjid","Overall % Correct", "Load 1 % Correct","Load 3 % Correct","Delay 1 % Correct","Delay 3 % Correct","Load 1 Delay 1 % Correct","Load 1 Delay 3 % Correct","Load 3 Delay 1 % Correct","Load 3 Delay 3 % Correct", 
                         "Overall % Correct - Inc No Resp", "Load 1 % Correct - Inc No Resp","Load 3 % Correct - Inc No Resp","Delay 1 % Correct - Inc No Resp","Delay 3 % Correct - Inc No Resp","Load 1 Delay 1 % Correct - Inc No Resp","Load 1 Delay 3 % Correct - Inc No Resp","Load 3 Delay 1 % Correct - Inc No Resp","Load 3 Delay 3 % Correct - Inc No Resp",
                         "Overall Avg Reaction Time (s)","Load 1 Reaction Time (s)","Load 3 Reaction Time (s)","Delay 1 Reaction Time (s)", "Delay 3 Reaction Time (s)","Load 1 Delay 1 Reaction Time (s)","Load 1 Delay 3 Reaction Time (s)","Load 3 Delay 1 Reaction Time (s)","Load 3 Delay 3 Reaction Time (s)","Cohort")
...
pdf(paste(todayf,"WM_behave_percor2.pdf",sep="_"))
...
for (i in colnames(data_table2)[cols])
{
  ...
  p2<-p+geom_bar(stat="identity",position=dodge)+
    ...
    coord_cartesian(ylim=c(60,100))+scale_fill_manual(values=c("light grey","gray34")) +
  ...
  print(p2)
  ggsave(paste(i,".png",sep=""),p2,width=12,height=12)

}
dev.off()

data_table3<-data_table2[,c(20:28)]
data_table3<-data_table3*1000
Cohort<-data_table2$Cohort
data_table3<-cbind(data_table3,Cohort)
colnames(data_table3)<-c("Overall Avg Reaction Time (ms)","Load 1 Reaction Time (ms)","Load 3 Reaction Time (ms)","Delay 1 Reaction Time (ms)", "Delay 3 Reaction Time (ms)","Load 1 Delay 1 Reaction Time (ms)","Load 1 Delay 3 Reaction Time (ms)","Load 3 Delay 1 Reaction Time (ms)","Load 3 Delay 3 Reaction Time (ms)","Cohort")

pdf(paste(todayf,"WM_behave_RT.pdf", sep="_"))

cols<-c(1:9)
for (i in colnames(data_table3)[cols])
{
  
  
  textSize <- 20
  dodge <- position_dodge(width=0.9)
  means<-(tapply(data_table3[,i], data_table3$Cohort, mean,na.rm=TRUE))
  
  sem<-tapply(data_table3[,i], data_table3$Cohort, sd,na.rm=TRUE)/sqrt(sqrt(length(data_table3[,i])))
  ...
  p2<-p+geom_bar(stat="identity",position=dodge)+
    ...
    coord_cartesian(ylim=c(700,1300))+scale_fill_manual(values=c("light grey","gray34"))+
    xlab(xlab) + ylab(ylab) + theme_bw(base_size=textSize)+theme(legend.position="none")+theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
  print(p2)
  ggsave(paste(i,".png",sep=""),p2,width=12,height=12)
}
...
#anova for rt w/ laod
#20160222-main effect of cohort & load, no interaction
WM_rt<-data_table2[,c(1,12:13,20)]
...
WM_del_rt<-data_table2[,c(1,14:15,20)]
