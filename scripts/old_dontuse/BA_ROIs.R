...
test<-read.delim(file="/Volumes/Phillips/P5/scripts/txt/delay_ld1_BA_spheres.txt",header=F)
test2<-test[,c(1,2,4,6,8,10,12,14,16)]
...
colnames(test2)<-c("ID","LBA17_dly_ld3","LBA40_dly_ld3","LBA46_dly_ld3","LBA9_dly_ld3","RBA17_dly_ld3","RBA40_dly_ld3","RBA46_dly_ld3","RBA9_dly_ld3")
...
onesIwant<-match(test2$ID,subj$MRID)
#get a new data table with only subs I want
subj2<-subj[onesIwant,]

data_ROIs<-merge(test2,subj2,by.x="ID",by.y="MRID")
#take out the crappy controls
data_ROIs<-data_ROIs[c(1:9,17),]
...
pdf(paste(todayf,"WM_BA_ROIs_ld1.pdf",sep="_"))
...
for (i in colnames(data_ROIs)[cols])
{
  ...
  p2<-p+geom_bar(stat="identity",position=dodge)+
    ...
    coord_cartesian(ylim=c(-30,40))+scale_fill_manual(values=c("light grey","gray34")) +
