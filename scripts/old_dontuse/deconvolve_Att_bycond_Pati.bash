...
# Edited 02/03/16 by Pati - fixing total_Popout.1D to order timing
...
cd $contrasts
model="SPMG1"  #"BLOCK(1.0,1)" # cue  is 1.0s long
pmodel="SPMG1"  #"BLOCK(0.6,1)" # 01/28/16 average RT is 609 ms long

prefix=bycond_caonly_tprobe_SPMG1
...
if [ $nInc -ne 2 ]; then
  ...
  3dDeconvolve \
    ...
    -num_stimts 8 \
    -stim_times 1 $oneddir/cue_tPopout_cCorrect.1D   "$model"  -stim_label 1 Popout_ca \
    -stim_times 2 $oneddir/cue_tHabitual_cCorrect.1D "$model"  -stim_label 2 Habitual_ca \
    -stim_times 3 $oneddir/cue_tFlexible_cCorrect.1D "$model"  -stim_label 3 Flexible_ca \
    ...
    -stim_times 8 $oneddir/total_Incorrect.1D "$model" -stim_label 8 total_incorrect \
    -num_glt 3 \
    -gltsym 'SYM:.5*Habitual_ca -.5*Popout_ca'                    -glt_label 1 HabvPop \
    -gltsym 'SYM:.5*Flexible_ca -.5*Popout_ca'                    -glt_label 2 FlexvPop \
    -gltsym 'SYM:.5*Flexible_ca -.5*Habitual_ca'                    -glt_label 3 FlexvHab \
    -overwrite \
